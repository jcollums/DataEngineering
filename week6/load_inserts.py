...
def load(conn, icmdlist):
    ...
    INSERT INTO {TableName} VALUES (%(CensusTract)s, %(State)s, %(County)s, %(Men)s, %(Women)s, %(Hispanic)s, %(White)s, %(Black)s, %(Native)s, %(Asian)s, %(Pacific)s, %(Citizen)s, %(Income)s, %(IncomeErr)s, %(IncomePerCap)s, %(IncomePerCapErr)s, %(Poverty)s, %(ChildPoverty)s, %(Professional)s, %(Service)s, %(Office)s, %(Construction)s, %(Production)s, %(Drive)s, %(Carpool)s, %(Transit)s, %(Walk)s, %(OtherTransp)s, %(WorkAtHome)s, %(MeanCommute)s, %(Employed)s, %(PrivateWork)s, %(PublicWork)s, %(SelfEmployed)s, %(FamilyWork)s, %(Unemployment)s);
        ''', icmdlist)
 
        cursor.execute(f"INSERT INTO {TableName}_final SELECT * FROM {TableName}")
...
def main():
    ...
    df = pd.read_csv('acs2015_census_tract_data_part1.csv')
    df = df.dropna()
    df.to_csv('test.csv', index=False, header=False)

    start = time.perf_counter()
    cur.copy_from(open('test.csv'), 'censusdatajcollums', ',')
